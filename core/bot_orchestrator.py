...
class BotOrchestrator:
    ...
    async def _market_data_listener(self, market_data_streams: list[str]):
        """Слухає ринкові дані (стакани) для всіх символів."""
        ...
        async with self.bsm.multiplex_socket(market_data_streams) as socket:
            while True:
                try:
                    ...
                    if msg and 'e' in msg and 'm' in msg: # Повідомлення про помилку
    ...
    async def _user_data_listener(self):
        """Слухає потік даних користувача (ордери, баланс)."""
    ...
    async def _handle_user_data_message(self, msg: dict):
        ...
        if status == 'FILLED' and order_type in ['LIMIT', 'MARKET'] and client_order_id in self.pending_sl_tp:
            ...
            if not sl_tp_prices:
                logger.error(f"[{symbol}] Не вдалося розрахувати SL/TP для ціни входу {actual_entry_price}.")
                # Аварійно закриваємо позицію, якщо не можемо розрахувати SL/TP
    ...
    async def start(self):
        ...
        async with BinanceClient() as client:
            ...
            # Фільтруємо конфігурації стратегій, щоб вони відповідали топ-символам
            active_strategies = [
            ...
            # Створюємо та запускаємо окремі задачі для кожного потоку
